<?php
    ...
    function list_bag()
    {
        ...
        for ($i = 0 ; $i < $nb_items; $i++)
            echo("<form action='modify_qt.php'><input type='number' name='qty' step='1' value='".$_SESSION['bag']['qty'][$i]."' max='99' min='1'>"."x. ".$_SESSION['bag']['name'][$i]." - Price : ".$_SESSION['bag']['price'][$i]."<input type='submit' name='modify' value='Modify'></form>".'<form method="POST" action="del_item.php"><input type="submit" name="del_item" value="Delete"><input type="hidden" name="id" value="'.$_SESSION["bag"]["id"][$i].'"></form>'."<br /><br />");
        echo("\nTotal : ".calc_total());
    }
    session_start();
    list_bag();
